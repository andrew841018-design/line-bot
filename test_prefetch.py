...
def test_js_domain_detection():
    ...
    for name, url in must_not_match:
        check(
            f"{name} (不該匹配)",
            not bool(_JS_RENDERED_DOMAINS.search(url)),
...
def test_db_clean():
    ...
    cur.execute(
        "SELECT * FROM persona_notes WHERE content LIKE '%tiktok%' OR content LIKE '%TikTok%' OR content LIKE '%忽略%'"
    )
    ...
    cur.execute(
        "SELECT * FROM filter_rules WHERE pattern LIKE '%tiktok%' OR pattern LIKE '%TikTok%'"
    )
...
def test_system_prompt():
    ...
    # 禁止用語
    check(
        "Rule: 不說「我跳過」", "我跳過" in _SYSTEM_PROMPT
    )  # 出現在「不要說」的上下文
    ...
    check("Rule: 不插嘴", "不需要回應的訊息" in _SYSTEM_PROMPT)
...
def _check_gemini_reply(url: str, name: str):
    ...
    try:
        ...
        check(
            f"{name}: 無 citation 標籤",
            "[cite:" not in reply and "[BROWSING_TOOL_" not in reply,
            f"殘留 tag: {reply[:80]}",
        )
