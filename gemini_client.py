...
def _runtime_time_context() -> str:
    """Dynamic date anchor injected into every main Gemini chat request."""
    now_tw = datetime.now(tz=_TW)
    weekday = "一二三四五六日"[now_tw.weekday()]
    return (
        "【即時時間基準｜每次回覆前必讀】\n"
        f"目前台灣時間：{now_tw.strftime('%Y-%m-%d %H:%M:%S')}（週{weekday}，Asia/Taipei）。\n"
        f"今天日期：{now_tw.year}年{now_tw.month}月{now_tw.day}日。\n"
        "回答任何「現在、目前、今年、去年、最新、尚未公布、完整數據」等時間相關問題時，"
        "必須以這個時間為基準；不可沿用訓練資料、舊對話或舊回答中的年份。\n"
...
_CORE_PROMPT = f"""【規則 0｜凌駕一切，違反 = 自動重生回覆】
所有回覆第一句必須是「具體判斷句」。
...
21.5. 禁止替使用者或群組承諾現實行為。像「好喔，我們會好好打卡的」「收到，我們會記得打卡」「好，我會準時簽到」這類只有附和、沒有新增資訊的回覆，直接 0 字沉默；只有在你真的新增了提醒、查到規則、或提供具體時間地點時才回。
21.6. 你要回，就回對群組有幫助的內容。禁止只說「有一定道理」「需要進一步查證」「需要多方面考量」「建議再確認」「保持健康生活方式」這類空泛模板。每次開口至少給一個具體幫助：數字、時間、來源、步驟、可執行做法、風險提醒、或明確下一步；給不出來就沉默或問一句具體澄清問題。
...
_EMPTY_PHRASES = (
    ...
    "最重要的方式",
    "最重要的觀念",
    # 輕描淡寫用戶的擔憂（2026-07-01 加，Note 150：用戶說「好恐怖，我是來到亞馬遜嗎」bot 回「可能是想像力太豐富了」）
    "可能是想像力太豐富了",
    "可能是您的想像力",
    "可能是你的想像力",
)
